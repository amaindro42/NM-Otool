...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/09/27 14:18:19 by amaindro          #+#    #+#             */
/*   Updated: 2017/10/02 17:32:44 by amaindro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "nm.h"

void	get_syms_32_reverse(struct load_command *lc, char *ptr,
...
char	**get_sect_32_reverse(struct load_command *lc, void *file_end)
{
	int						n;
	char					**sect_array;
...
void	nm_32_reverse(char *ptr, void *file_end)
{
	...
	while (++i < (int)header->ncmds)
	{
		...
		{
			sect_array[1] = get_sect_32_reverse(lc, file_end);
		...
		if (lc->cmd == LC_SYMTAB)
			get_syms_32_reverse(lc, ptr, sect_array[0], file_end);
